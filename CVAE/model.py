...
class SimpleEncoder(nn.Module):
    def __init__(self, config, scene_vocab_size, object_vocab_size):
        super(SimpleEncoder, self).__init__()
        self.factor_embedding_1 = nn.Embedding(scene_vocab_size, config.n_embd // 2)
        self.factor_embedding_2 = nn.Embedding(object_vocab_size, config.n_embd // 2)
        # self.factor_embedding_1 = nn.Embedding(object_vocab_size, config.n_embd)
        # self.factor_embedding_2 = nn.Embedding(object_vocab_size, config.n_embd)
        ...
        self.cond1 = ConditionalLayerNorm(config.n_embd)
        self.cond2 = ConditionalLayerNorm(config.n_embd)
        # self.cond1 = nn.Sequential(
        #     nn.Linear(config.n_embd, config.n_embd),
        #     nn.LayerNorm(config.n_embd, eps=1e-5)
        # )
        # self.cond2 = nn.Sequential(
        #     nn.Linear(config.n_embd, config.n_embd),
        #     nn.LayerNorm(config.n_embd, eps=1e-5)
        # )
        self.mean1 = nn.Linear(config.n_embd, config.n_embd // 2)
        self.mean2 = nn.Linear(config.n_embd, config.n_embd // 2)
        self.logvar1 = nn.Linear(config.n_embd, config.n_embd // 2)
        self.logvar2 = nn.Linear(config.n_embd, config.n_embd // 2)

    def forward(self, input_ids=None, attention_mask=None, scene_labels=None, object_labels=None):
        scene_embedding = self.factor_embedding_1(scene_labels)
        object_embedding = self.factor_embedding_2(object_labels)
        eps1 = torch.randn_like(scene_embedding)
        eps2 = torch.randn_like(object_embedding)
        scene_states = self.cond1(self.mlp1(torch.cat((scene_embedding, eps1), dim=-1)), scene_embedding)
        object_states = self.cond2(self.mlp2(torch.cat((object_embedding, eps2), dim=-1)), object_embedding)
        # scene_states = self.cond1(self.mlp1(scene_embedding))
        # object_states = self.cond2(self.mlp2(object_embedding))
        mean1 = self.mean1(scene_states)
        mean2 = self.mean2(object_states)
        logvar1 = self.logvar1(scene_states)
        logvar2 = self.logvar2(object_states)
        return torch.cat((mean1, mean2), dim=-1), torch.cat((logvar1, logvar2), dim=-1)
...
class VAEModel(GPT2LMHeadModel):
    def __init__(self, config, add_input=False, add_attn=False, add_softmax=False, attn_proj_vary=False, learn_prior=False):
        ...
        label_list = json.load(open("../resources/label.json", "r", encoding="utf-8"))
        self.scene_vocab_size, self.object_vocab_size = len(label_list["Scene"]), len(label_list["Object"])
        ...
        if self.learn_prior:
            ...
            self.encoder_prior = SimpleEncoder(config, self.scene_vocab_size, self.object_vocab_size)
    ...
    def forward(
        ...
        posterior = self.encoder(input_ids=y_tokens, attention_mask=y_mask)[0]

        if self.learn_prior:
            # prior_mean, prior_logvar = self.encoder_prior(input_ids=x_tokens, attention_mask=x_mask)[:2]
            prior = self.encoder_prior(input_ids=x_tokens, attention_mask=x_mask, scene_labels=scene_labels, object_labels=object_labels)[0]
        else:
        ...
        z = prior if from_prior else posterior
...
class Discriminator(nn.Module):
    ...
    def get_loss(self, posterior, prior, scene_labels, object_labels):
        eps1 = torch.randn_like(posterior) * self.noise_weight
        eps2 = torch.randn_like(prior) * self.noise_weight
        logits_real = self(posterior + eps1, scene_labels, object_labels)
        logits_fake = self(prior + eps2, scene_labels, object_labels)
