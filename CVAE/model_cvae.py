...
class SimpleEncoder(nn.Module):
    def __init__(self, config):
        ...
        scene_vocab_size, object_vocab_size = len(label_list["Scene"]), len(label_list["Object"])
...
class VAEModel(GPT2LMHeadModel):
    def __init__(self, config, add_input=False, add_attn=False, add_softmax=False, attn_proj_vary=False, learn_prior=False):
        ...
        if self.learn_prior:
            ...
            # self.encoder_prior = SimpleEncoder(config)
    ...
    def forward(
        ...
        if self.learn_prior:
            ...
            # prior_mean, prior_logvar = self.encoder_prior(input_ids=x_tokens, attention_mask=x_mask, scene_labels=scene_labels, object_labels=object_labels)[:2]
        else:
