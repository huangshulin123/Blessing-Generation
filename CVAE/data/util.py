...
def encode_tuple(tokenizer, t):
    return tokenizer.encode(t[0], add_special_tokens=False), tokenizer.encode(t[1], add_special_tokens=False), tokenizer.encode(t[2], add_special_tokens=False), (t[3] if len(t) > 3 else None)


def truncate_tuple(truncator, t):
    return truncator(t[0]), truncator(t[1]), truncator(t[2]), (t[3] if len(t) > 3 else None)
    ...
    def f(text_raw_dict):
        ...
        if data_type == 't0':  # x, y, y
            if 'prompt' in text_raw_dict:
                ...
                pp = story.split('<newline><newline>')
                story = '\n\n'.join(pp)
            pattern = re.compile(r"Send this blessing to (.+?) for (.+?):")
            _object, scene = pattern.match(summary).groups()
            return summary + tokenizer.eos_token, story + tokenizer.eos_token, tokenizer.eos_token + story + tokenizer.eos_token, (scene, _object)
            ...
            return summary + tokenizer.eos_token, summary_story, tokenizer.eos_token + summary_story, (scene, _object)
...
def collate_fn(samples):
    ...
    if label_dict is None:
        ...
        label_dict = {"Scene": {v: i for i, v in enumerate(label_list['Scene'])}, "Object": {v: i for i, v in enumerate(label_list['Object'])}}
    ...
    output = x_mask, x, y_mask, y, input[:, :-1], input[:, 1:].contiguous(), input_mask[:, 1:]
    if samples[0][3] is None:
        return output
    scene_labels = torch.LongTensor([label_dict['Scene'][ss[3][0]] for ss in samples])
    object_labels = torch.LongTensor([label_dict['Object'][ss[3][1]] for ss in samples])
    # print(x_mask, x, y_mask, y, input_mask, input, scene_labels, object_labels)
    return output + (scene_labels, object_labels)
...
def prepare_dataset(data_dir, dataset_name, tokenizer, train_bsz, train_seq_len, val_bsz, val_seq_len, test_bsz=1,
    ...
    if dataset_name == 'wp':
        ...
        print('Loading arxiv dataset...')
        data_abs = os.path.join(data_dir, 'blessing/train_content.txt')
        data_titles = os.path.join(data_dir, 'blessing/train_title.txt')
        with open(data_abs, errors='ignore') as fp:
            abs = fp.readlines()
        with open(data_titles, errors='ignore') as ft:
            titles = ft.readlines()

        assert len(titles) == len(abs),f"{len(titles)} {len(abs)}"
        ai_data = [('ai', t.strip(), p.strip()) for t, p in zip(titles, abs) if t.strip() != '' and p.strip() != '']

        data_abs = os.path.join(data_dir, 'blessing/val_content.txt')
        data_titles = os.path.join(data_dir, 'blessing/val_title.txt')
        with open(data_abs, errors='ignore') as fp:
            abs = fp.readlines()
        with open(data_titles, errors='ignore') as ft:
            titles = ft.readlines()
        ...
        data_abs = os.path.join(data_dir, 'blessing/test_content.txt')
        data_titles = os.path.join(data_dir, 'blessing/test_title.txt')
        with open(data_abs, errors='ignore') as fp:
            abs = fp.readlines()
        with open(data_titles, errors='ignore') as ft:
            titles = ft.readlines()
