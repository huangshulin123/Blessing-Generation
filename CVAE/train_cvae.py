...
def main():
    ...
    parser.add_argument('--out-dir', type=str, default='out-vae-gpt2')
    ...
    args = parser.parse_args('test --batch-sizes 16 --seq-lens 1024 '
    ...
    lr_schedule = switch_schedule(linear_schedule(args), 16 / batch_schedule[-1][0],
                                  int(args.iterations * args.switch_time))
    VAE = VAE.to(device)
    VAE.train()
    ...
    def val_step(val_loader):
        ...
        with tqdm(total=min(len(val_loader), max_val_batches)) as pbar:
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, *_) in enumerate(val_loader):
    ...
    def generate(test_loader, num_iters):
        ...
        # test_iter = iter(test_loader); x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask = next(test_iter)
        with tqdm(total=len(test_loader)) as pbar:
            for i_test, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, *_) in enumerate(
                    test_loader):
    ...
    while num_iters < args.iterations:
        ...
        test_interval = round(len(train_loader) / 2)
        # train_iter = iter(train_loader); x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask = next(train_iter)
        with tqdm(total=len(train_loader)) as pbar:
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, *_) in enumerate(train_loader):
                # print(f"{x_mask=}, {x_tokens=}, {y_mask=}, {y_tokens=}, {input_tokens=}, {target_tokens=}, {mask=}")
                # print("FUCK"); exit()
