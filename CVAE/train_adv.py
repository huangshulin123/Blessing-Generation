...
def compute_loss(device, model, x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels, loss_fn, beta):
    ...
    outputs = model(input_ids=input_tokens, attention_mask=mask, x_mask=x_mask, x_tokens=x_tokens, y_mask=y_mask, y_tokens=y_tokens, scene_labels=scene_labels, object_labels=object_labels)
    logits = outputs[0]
    kl_loss = outputs[-1]
...
def train_step(device, model, optimizer, x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels, loss_fn, beta, model_type, discriminator, optimizer_generator, optimizer_discriminator):
    output = []
    ...
    optimizer.zero_grad()
    loss, ce_loss, kl_loss = compute_loss(device, model, x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels, loss_fn, beta)
    with amp.scale_loss(loss, optimizer) as scaled_loss:
        scaled_loss.backward()
        torch.nn.utils.clip_grad_norm_(amp.master_params(optimizer), 5.0)  # max_grad_norm=1.0
    # loss.backward()
...
def sample_sequence(model, tokenizer, length, batch_size=None, x_mask=None, x_tokens=None, y_mask=None, y_tokens=None, scene_labels=None, object_labels=None, temperature=1, top_k=100, top_p=0.95, device='cuda', sample=True, eos_token=None, model_type='cvae', from_prior=True):
    x_mask, x_tokens, scene_labels, object_labels, y_mask, y_tokens = x_mask.to(device), x_tokens.to(device), scene_labels.to(device), object_labels.to(device), y_mask.to(device), y_tokens.to(device)

    with torch.no_grad():
        if model_type == 'cvae':
            if from_prior:
                prior = model.encoder_prior(input_ids=x_tokens, attention_mask=x_mask, scene_labels=scene_labels, object_labels=object_labels)[0]
                z = prior
            else:
                posterior = model.encoder(input_ids=y_tokens, attention_mask=y_mask)[0]
                z = posterior
        else:
        ...
        # print(x_tokens.shape, z.shape, scene_labels.shape)
        # exit()
        ...
        # prev = torch.tensor([50256], device=device).repeat(batch_size, 1)
        # mem = None
...
def main():
    ...
    parser.add_argument('--iterations', type=int, default=18008 * 10)  # wp 850001  wi 300001 ax 300001 yp 800001
    parser.add_argument('--dataset', type=str, default='ax', choices=['ax', 'yp', 'wp', 'wi',"bl"], help="Dataset to use for training")
    ...
    parser.add_argument('--out-dir', type=str, default='out-gan-prompt')
    ...
    args = parser.parse_args('test --batch-sizes 16 --seq-lens 256 '
    ...
    for name, parameter in VAE.named_parameters():
        ...
        ['factor_embedding_1', 'factor_embedding_2', 'mlp1', 'mlp2', 'cond1', 'cond2', 'mean1', 'mean2', 'logvar1', 'logvar2', ]
    ...
    train_loader, val_loader, test_loader = prepare_dataset(
        ...
        16, batch_schedule[cur_b_schedule][1],
    ...
    lr_schedule = switch_schedule(linear_schedule(args), 16 / batch_schedule[-1][0], int(args.iterations * args.switch_time))
    VAE.to(device)
    VAE.train()
    discriminator = Discriminator(VAE.scene_vocab_size, VAE.object_vocab_size, config.n_embd, config.n_embd).to(device)

    optimizer = AdamW(VAE.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_schedule)

    optimizer_generator = AdamW(VAE.encoder_prior.parameters(), lr=1e-3, betas=(0.9, 0.99), weight_decay=1e-4)
    optimizer_generator = ScheduledOptim(optimizer_generator, warmup_steps=5000, max_lr=1e-4, min_lr=1e-7, beta=0.55)

    optimizer_discriminator = AdamW(discriminator.parameters(), lr=1e-3, betas=(0.5, 0.99), weight_decay=1e-4)
    optimizer_discriminator = ScheduledOptim(optimizer_discriminator, warmup_steps=5000, max_lr=1e-4, min_lr=1e-7, beta=0.5)

    # VAE, optimizer = amp.initialize(VAE, optimizer, opt_level=args.fp16_opt_level)
    # VAE, [optimizer, optimizer_generator, optimizer_discriminator] = amp.initialize(VAE, [optimizer, optimizer_generator, optimizer_discriminator], opt_level=args.fp16_opt_level)
    ...
    def val_step(val_loader):
        ...
        with tqdm(total=min(len(val_loader), max_val_batches)) as pbar:
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask) in enumerate(val_loader):
                # print(f"{x_mask=}, {x_tokens=}, {y_mask=}, {y_tokens=}, {input_tokens=}, {target_tokens=}, {mask=}")
                ...
                story = [tokenizer.decode(target_tokens[i, :].tolist()) for i in range(n)]
    ...
    def generate(test_loader, num_iters):
        ...
        with tqdm(total=len(test_loader)) as pbar:
            ...
            for i_test, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels) in enumerate(test_loader):

                # if i_test >= 10: break

                # length = -1
                length = args.seq_lens[0]
                if length == -1:
                ...
                storys = [tokenizer.decode(target_tokens[i, :].tolist()) for i in range(n)]
                ...
                # storys_str = [s[:s.find("<|endoftext|>")] if "<|endoftext|>" in s else s for s in storys]

                # with torch.no_grad():
                #     prior_mean, prior_logvar, *_ = VAE.encoder_prior(input_ids=x_tokens.to(device), attention_mask=x_mask.to(device))
                #     latent_params.append({"mean": prior_mean, "logvar": prior_logvar})
                ...
                for _ in range(args.nsamples // args.batch_size):
                    ...
                    for i in range(len(out)):
                        ...
                        eff_samples.append((text, bleu4, rouge_scores, text2))
                ...
                for i in range(len(eff_samples)):
                    ...
                    samples_file.write("=" * 40 + " Generated (Posterior) " + "=" * 40)
                    samples_file.write('\n' * 2)
                    samples_file.write(eff_samples[i][-1])
                    samples_file.write('\n' * 4)
                    samples_file.flush()
        ...
        # torch.save(latent_params, os.path.join(save_folder, 'latent-' + '%07d' % num_iters + '.pkl'))
    ...
    while num_iters < args.iterations:
        ...
        test_interval = round(len(train_loader) / 4)
        ...
        with tqdm(total=len(train_loader)) as pbar:
            ...
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels) in enumerate(train_loader):
                ...
                # output = train_step(device, VAE, optimizer, x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels, loss_fn, beta, args.model_type)
                # output = train_step(device, VAE, optimizer, x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, scene_labels, object_labels, loss_fn, beta, args.model_type, discriminator, optimizer_generator, optimizer_discriminator)
                # loss, ce_loss, kl_loss = output[-1]
                input_tokens, target_tokens, mask, x_mask, x_tokens, y_mask, y_tokens, scene_labels, object_labels = input_tokens.to(device), target_tokens.to(device), mask.to(device), x_mask.to(device), x_tokens.to(device), y_mask.to(device), y_tokens.to(device), scene_labels.to(device), object_labels.to(device)

                optimizer.zero_grad()
                from_prior = (num_iters % 2 == 1)
                outputs = VAE(input_ids=input_tokens, attention_mask=mask, x_mask=x_mask, x_tokens=x_tokens, y_mask=y_mask, y_tokens=y_tokens, scene_labels=scene_labels, object_labels=object_labels, from_prior=from_prior)
                logits = outputs[0]
                num_logits = logits.size(-1)
                if mask is not None:
                    mask = mask.type(torch.bool)
                ...
                if num_iters > -1:
                    ...
                    for i in range(dis_step):
                        with torch.no_grad():
                            prior = VAE.encoder_prior(input_ids=x_tokens, attention_mask=x_mask, scene_labels=scene_labels, object_labels=object_labels)[0]
                            posterior = VAE.encoder(input_ids=y_tokens, attention_mask=y_mask)[0]
                        dis_loss += discriminator.get_loss(posterior, prior, scene_labels, object_labels)
                    dis_loss /= dis_step
                    dis_loss.backward()
                    ...
                    prior = VAE.encoder_prior(input_ids=x_tokens, attention_mask=x_mask, scene_labels=scene_labels, object_labels=object_labels)[0]
                    posterior = VAE.encoder(input_ids=y_tokens, attention_mask=y_mask)[0]
                    gen_loss = discriminator.get_loss(posterior, prior, scene_labels, object_labels)
                    gen_loss.backward()
                    torch.nn.utils.clip_grad_norm_(VAE.encoder_prior.parameters(), clip_grad_norm)
                    optimizer_generator.step()
                    kl_loss = gen_loss.item()


                lr = scheduler.get_last_lr()[0]
                # Log to Tensorboard
                # t_writer.add_scalar('loss', loss, num_iters)
                t_writer.add_scalar('loss', ce_loss, num_iters)
